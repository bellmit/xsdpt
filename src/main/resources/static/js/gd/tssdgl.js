    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            for(var i=0;i<data.length;i++){
                ...
                html+='<td>'+data[i].ssdrmc+'</td>'
                html+='<td>'+data[i].sfzhm+'</td>'
                html+='<td>'+data[i].hm+'</td>'
                html+='</tr>'
            }
            $("#ssdrxx_tbody").empty();
            $("#ssdrxx_tbody").append(html);
        }
    })
}

function add_ssdrxx() {
    var ssdrmc=$("#ssdrmc").val()
    var sfzhm=$("#sfzhm").val()
    var hm=$("#dh").val()
    var params={
        yysdbh:yysdbh,
